# translating the maple worksheet into executable fortran code
# - 3 directories(paths) have to be specified below. The format is:
# on PCs: "C:\\xxx\\yyy"; on Macs: "Macintosh HD:xxx:yyy"
# - This version overwrites "switches.f". You can avoid this by setting iswitch to a value other than 1. 
# However, if you don't create switches.f automatically, a dummy subroutine switches.f of the form "subroutine switches(j); integer j; return;end" is required when compiling the fortran code
restart ;
iswitch:=0;
# point to folder where acglib resides
if searchtext("Maple 6", kernelopts(version)) = 1 then
  ...
  libname := "C:\\Program Files\\BRNSPackage\\acglib-Maple10", "C:\\Program Files\\Maple 10\\lib", libname ;
end if; 
...
# point to spread.m
currentdir("C:\\Program Files\\BRNSPackage"):
read "spread.m" ;
# possibility to overwrite the directory where the fortran files are saved to
# if commented out, it goes to the directory that was specified in the worksheet when creating spread.m
dir_f:="C:\\Program Files\\BRNSPackage\\GeneratedFortranFiles":
